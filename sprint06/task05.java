...
public class MyUtils {
    public Map<String, Double> averageRating(List<Caffee> coffees) {
        List<Caffee> copyCoffees = new ArrayList<>(new HashSet<>(coffees));
        List<Caffee> espressos = new ArrayList<>();
        List<Caffee> cappuccinos = new ArrayList<>();

        for (Caffee caffee : copyCoffees) {
            if (caffee instanceof Espresso) {
                espressos.add(caffee);
            }

            if (caffee instanceof Cappuccino) {
                cappuccinos.add(caffee);
            }
        }

        copyCoffees.removeAll(espressos);
        copyCoffees.removeAll(cappuccinos);

        List<Caffee> caffees = new ArrayList<>(copyCoffees);

        Map<String, Double> result = new LinkedHashMap<>();
        List<List<Caffee>> temp = new ArrayList<>();

        temp.add(espressos);
        temp.add(cappuccinos);
        temp.add(caffees);

        for (List<Caffee> caffeeList : temp) {
            if (!caffeeList.isEmpty()) {
                int rating = 0;

                int j = 0;

                for (; j < caffeeList.size(); j++) {
                    rating += caffeeList.get(j).getRating();
                }

                result.put(caffeeList.get(j - 1).getClass().getSimpleName(), rating / (double) caffeeList.size());
            }
        }
